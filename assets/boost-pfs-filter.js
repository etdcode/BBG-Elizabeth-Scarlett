...
var boostPFSFilterConfig = {
	general: {
		...
		placeholderProductGridItemClass: 'boost-pfs-filter-product-item custom-item boost-pfs-filter-product-item-grid boost-pfs-filter-grid-width-3 boost-pfs-filter-grid-width-mb-2',
		enableOTP: true,
...
(function() {
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		// Add TAG New in
		itemHtml = itemHtml.replace(/{{itemTagNewin}}/g, buildTagNewin(data));
	...
	function buildImages(data) {
        ...
        for(tag of data.tags){
			if(tag === 'monogram' ){
				html += `<div class="tagMono">Monogram Me</div>`
			}
		}
		html += '<img class="boost-pfs-filter-product-item-main-image "' +
	...
	// BUILD New in WITH TAGS
	function buildTagNewin(data) {
		var tagLabel = '';
			if (data.tags) {
				...
				for (var i = 0; i < data.tags.length; i++) {
						...
						if (tag.indexOf("new-in") !== -1) {
							...
							var preTagLabel = boostPFSTemplate.tagLabelHtml;
							tagLabel = '<span class="boost-pfs-filter-label tag new-in">NEW IN</span>';
						}
					}
				}
			...
			if (typeof data.options_with_values != 'undefined' && data.options_with_values.length > 0) {
				...
				if (swatchArr.length > 0) {
					...
					for (var sIndex = 0; sIndex < swatchLimit; sIndex++) {
						...
						swatchesProductOptionHtml += '<li class="' + swatchName + '" value="' + swatchName + '">';
                        if(optionName == 'color' || optionName == 'colour'){
                            swatchesProductOptionHtml += '<div class="boost-pfs-product-item-tooltip">' + swatchName + '</div>';
                            swatchesProductOptionHtml += '<span class="js-color-' + swatchName + '" tabindex="0" aria-label="' + swatchName + '" ' +
	...
	// Build Color Swatches
	function eventColorSwatches(event, action) {
		let getArr = action.reverse();
		jQ('.txt-swatch').text('+ Colours');
		setTimeout(function(){
			jQ('.boost-pfs-filter-product-item.custom-item').each(function() {
				var filterColor = [];
				jQ(this).find('.boost-pfs-filter-item-swatch-color li').each(function(){
					filterColor.push( jQ(this).attr('value') );
				});
				jQ.each(getArr, (index, item) => {
					if(filterColor.indexOf(item) > -1){
						$(this).find('li.' + item).addClass('active');
						const getImg = $(this).find('.js-color-' + item).attr('data-img');
						jQ(this).find('.boost-pfs-filter-product-item-main-image').attr('src',getImg).attr('srcset', getImg);
						return false;
					}
				});
			});
	...
	Filter.prototype.afterRender = function(data) {
		...
		var $getitemColor = jQ('.boost-pfs-filter-option-item-list-swatch').children("li");
		if (urlFilter.indexOf('pf_opt_color') <= 0 && $getitemColor.length  > 6){
			$getitemColor.slice(6).hide();  
			$getitemColor.eq(6).after(
				$('<li class="boost-pfs-filter-option-item show-panel">+</li>').click(function(){
				    jQ(this).remove();
				    $getitemColor.show();
				})
			); 
		...
		if (urlFilter.indexOf('pf_opt') > 0){
			jQ('.remove-filter-all').removeClass('hide');
		}else{
			jQ('.remove-filter-all').addClass('hide');
		}
		jQ(document).on('click', '.remove-filter-all', function() {
			jQ('.remove-filter-all').addClass('hide');
			jQ('.boost-pfs-filter-clear-all').trigger('click');
        });
		jQ(document).on('click', '.boost-pfs-filter-tree-mobile-button button', function() {
			jQ('.boost-pfs-filter-tree-content').find('.sortby-sp').remove();
			jQ('.sortby-sp').clone().insertBefore('.boost-pfs-filter-options-wrapper');
		});
		jQ(document).on('click', '.sortby-sp ul li', function() {
			jQ('.sortby-sp ul li').removeClass('active');
			jQ(this).addClass('active');
			const dataSort = jQ(this).find('a').attr('data-sort');
			jQ('.boost-pfs-filter-filter-dropdown a[data-sort="'+ dataSort  +'"]').trigger('click');
        });
		jQ(document).on('click', '.close-filter', function() {
			jQ('.boost-pfs-filter-close').trigger('click');
		});
		...
		let dataColor = new URL(urlFilter).searchParams;
    	let dataColorArr =   dataColor.getAll('pf_opt_color'); 

		console.log('dataColorArr: '  + dataColorArr);

		// Build Event Color Swatch
		if (boostPFSThemeConfig.custom.swatch_color_event_change_image != 'none') {
			eventColorSwatches(boostPFSThemeConfig.custom.swatch_color_event_change_image, dataColorArr);
		}
		...
		jQ(window).on("resize", function(){
			jQ('.boost-pfs-container-default-box').find('.card_item').remove();
			jQ('.boost-pfs-container-default-box').find('.card_video').remove();
			renderBannerList();
			renderCollectionCart();
		});
		function renderBannerList(){
			let width = jQ(document).width();
			let arrPos = [];
			const t1 =  jQ('.card_1').length;
			const t2 =  jQ('.card_2').length;
			const t4 =  jQ('.card_4').length;
			const t8 =  jQ('.card_8').length;

			if(width > 767){
				if(t2 && !t4 && !t8){
					arrPos = [7,8,14,0,19,20,25,0,30,32,34,37,41,42] ;
				}else if(!t2 && t4 & !t8){
					arrPos = [7,0,15,16,19,20,25,0,30,32,34,37,41,42] ;
				}else if(!t2 && !t4 & t8){
					arrPos = [7,0,15,0,16,17,22,25,30,32,34,37,41,42] ;
				}else if(t2 && t4 & !t8){
					arrPos = [7,8,14,15,18,19,24,0,29,31,33,36,40,41] ;
				}else if(t2 && !t4 & t8){
					arrPos = [7,8,14,0,15,16,21,24,29,31,33,36,40,41] ;
				}else if(!t2 && t4 & t8){
					arrPos = [7,0,11,12,15,16,21,24,29,31,33,36,40,41] ;
				}else if(!t2 && !t4 & !t8){
					arrPos = [7,0,15,0,16,17,22,0,27,29,31,34,38,39] ;
				}else{
					arrPos = [7,8,14,15,18,19,24,27,32,34,36,39,43,44] ;
				}

				jQuery.each( arrPos, function( i , val ) {
					i++;
					jQ('.card_'+ i ).clone().insertAfter('.boost-pfs-filter-products .boost-pfs-action-list-enabled:nth-of-type('+ val +')');
				});
				if (urlFilter.indexOf('display=list') > 0){
					const getUrlCol = $('.filter-customize').attr('data-url');
					window.location.href = getUrlCol; 
				}
			}else{
				if(!t1){
					arrPos = [4,6,9,12,0,15,16,17,18,20,21,22,25,26] ;
				}else{
					arrPos = [4,7,11,13,0,14,15,16,17,18,19,20,23,24] ;
				}
		...
		function renderCollectionCart(){
			...
			jQ('.boost-pfs-container-default-box').find('[js-product-card]').remove();
			if(jQ(document).width() > 767){
				cards.each((index, card) => {
					const number = parseInt(jQ(card).data('product-card-position')) - 1;
					jQ(card).clone().insertAfter(`.boost-pfs-filter-products .boost-pfs-action-list-enabled:nth-of-type(${number})`);
				})
			} else {
				cards.each((index, card) => {
					const number = parseInt(jQ(card).data('mobile-card-position')) - 1;
					jQ(card).clone().insertAfter(`.boost-pfs-filter-products .boost-pfs-action-list-enabled:nth-of-type(${number})`);
				})
			}
		}
		...
		renderValueFilter()
		const videoWrappers = document.querySelectorAll(".video-wrapper-collection")
		videoWrappers.forEach((videoWrapper) => {
			const video = videoWrapper.querySelector("video")
			const muteBtn = videoWrapper.querySelector(".mute-button-btn")
			const unmuteBtn = videoWrapper.querySelector(".unmute-button-btn")
     	...
     	jQ('.boost-pfs-filter-mobile-footer .boost-pfs-filter-show-result').hide();

		const getResult = jQ('.boost-pfs-filter-show-result').text();
		const  posgetResult = getResult.indexOf("(") + 1;
		const posgetResultLast = getResult.slice(posgetResult, -1)
		const htmlgetResult = '<button class="result-filter-mb">View '+ posgetResultLast +' items</button>'
		jQ('.result-filter-mb').remove();
		if(parseInt(posgetResultLast) > 1){
			const htmlgetResult = '<button class="result-filter-mb">View '+ posgetResultLast +' items</button>';
			jQ('.boost-pfs-filter-mobile-footer').append(htmlgetResult);
		}else{
			const htmlgetResult = '<button class="result-filter-mb">View '+ posgetResultLast +' item</button>';
			jQ('.boost-pfs-filter-mobile-footer').append(htmlgetResult);
		}
		jQ(document).on('click', '.result-filter-mb', function() {
			jQ('.boost-pfs-filter-show-result').trigger('click');
		});
	...
	function renderValueFilter(){
		let geturlFilter = window.location.href; 
		let stringFilterColor = [];
		let stringFilterSize = [];
		let valueParam = new URL(geturlFilter).searchParams;
    	 stringFilterColor =   valueParam.getAll('pf_opt_color'); 
		 stringFilterSize =   valueParam.getAll('pf_opt_size'); 
		 stringFilterSize = stringFilterSize.join(', ');
		 stringFilterColor = stringFilterColor.join(', ');
		jQ('.txt-value').remove();
		jQ('.boost-pfs-filter-option-color .boost-pfs-filter-option-title-text, .boost-pfs-filter-option-colour .boost-pfs-filter-option-title-text').append('<span class="txt-value"><span>:</span> '  + stringFilterColor.toString().replace(/,/g, ", ") +'</span>');
		jQ('.boost-pfs-filter-option-size .boost-pfs-filter-option-title-text').append('<span class="txt-value"><span>:</span> '  + stringFilterSize.toString().replace(/,/g, ", ") +'</span>')
	}
	// Swap Image
			...
			if (boostPFSThemeConfig.custom.hasOwnProperty('active_image_swap') &&
				...
				jQ(".boost-pfs-filter-product-item").each(function() {
					...
					if (typeof imgSelector.data('img-flip-src') != 'undefined' &&
						...
						html = '<img class="boost-pfs-filter-product-item-flip-image"' +
